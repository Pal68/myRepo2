...
IColor = cv.imread('./tmp/_11piece_metr6.jpg')
...
for i in range(1, r):
    for j in range(1, c):
        tmp = [abs(E1h[i, j]), abs(E1v[i, j]), abs(E2h[i, j]), abs(E2v[i, j]), abs(E3h[i, j]), abs(E3v[i, j]),
               abs(E4h[i, j]), abs(E4v[i, j]), abs(E5h[i, j]), abs(E5v[i, j]), abs(E6h[i, j]), abs(E6v[i, j])]
        maxv = max(tmp)
...
# -------------------Contrast - ------------------
# #
counts, graylevels = npy.histogram(I,
...
for m in range(0, (n - 1)):
    ...
    for k in range(1, len(deltaGt)):
        if ((all(deltaGt[k] >= Threshold)) and (theta1[k] >= (2 * m - 1) * math.pi / (2 * n)) and (
                theta1[k] < (2 * m + 1) * math.pi / (2 * n))):
            countk = countk + 1
            counti = counti + 1
    HD[m + 1] = countk
