...
def getLBPMatrix(img, chanel):
    ...
    for i in range(1, result_matrix.shape[0] - 1):
        for j in range(1, result_matrix.shape[1] - 1):
            ...
            data = ((result_matrix[i, j] - result_matrix[max(0, i - 1):min(result_matrix.shape[0], i + 2),
                                           max(0, j - 1):min(result_matrix.shape[1], j + 2)]) <= mean_delta * 0)
            c = np.zeros((3, 3))
            s = data.size
            match s:
                case 4:
...
def getGrayLBPMatrix(img):
    ...
    for i in range(gray.shape[0]):
        for j in range(gray.shape[1]):
            # Calculate LBP for red channel
            data = ((gray[i, j] - gray[max(0, i - 1):min(gray.shape[0], i + 2),
                                  max(0, j - 1):min(gray.shape[1], j + 2)]) >= 0)
            c = np.zeros((3, 3))
            s = data.size
            match s:
                case 4:
...
def getLBPPropsForPieceWithOverlap(matr, blok_size, tmp_name):
    ...
    for ii in range(0, end_h_iteration):
        for jj in range(0, end_w_iteration):
            ...
            for k in range(0, len(rgb_props)):
                gray_lbp_list.append(rgb_props[k])
...
def getLBPPropsForPiece(matr, blok_size, tmpName):
    ...
    for ii in range(0, end_h_iteration):
        for jj in range(0, end_w_iteration):
            ...
            gray_blok_matrix = getLBPMatrix(cyr_blok_matrix, 3)
            tmpLBPMatrix[ii * gray_blok_matrix.shape[0]:ii * gray_blok_matrix.shape[0] + gray_blok_matrix.shape[0],
            jj * gray_blok_matrix.shape[1]:jj * gray_blok_matrix.shape[1] + gray_blok_matrix.shape[
                1]] = gray_blok_matrix
...
def getLBPPropsForLayer(matr, blokSize):
    ...
    for ii in range(0, endHIteration):
        for jj in range(0, endWIteration):
            cyrBlokMatrix = copyPartMatrix(matr, jj * blokSize, jj * blokSize + (blokSize - 1), ii * blokSize,
                                           ii * blokSize + (blokSize - 1))
        ...
        if wOver > 3:
            cyrBlokMatrix = copyPartMatrix(matr, matr.shape[1] - wOver + 1, matr.shape[1] - 1, ii * blokSize,
                                           ii * blokSize + (blokSize - 1))
            cyrBlokMatrix = getGrayLBPMatrix(cyrBlokMatrix)
            cyrLBPHist, cyrLBPBin = getUniFormLBPHist(cyrBlokMatrix)
    ...
    if hOver > 3:
        for jj in range(0, endWIteration):
            cyrBlokMatrix = copyPartMatrix(matr, jj * blokSize, jj * blokSize + (blokSize - 1),
                                           matr.shape[0] - hOver + 1, matr.shape[0] - 1)
            cyrBlokMatrix = getGrayLBPMatrix(cyrBlokMatrix)
            cyrLBPHist, cyrLBPBin = getUniFormLBPHist(cyrBlokMatrix)
            # cyrPiecePropVector[(ii+jj) * 57:(ii+jj) * 57 + 57] = cyrLBPHist
            for k in range(0, cyrLBPHist.shape[0]):
                lbpList.append(cyrLBPHist[k])
    if wOver > 3:
        cyrBlokMatrix = copyPartMatrix(matr, matr.shape[1] - wOver + 1, matr.shape[1] - 1, matr.shape[0] - hOver + 1,
                                       matr.shape[0] - 1)
        cyrBlokMatrix = getGrayLBPMatrix(cyrBlokMatrix)
        cyrLBPHist, cyrLBPBin = getUniFormLBPHist(cyrBlokMatrix)
        # cyrPiecePropVector[(ii+jj) * 57:(ii+jj) * 57 + 57] = cyrLBPHist
...
def get_RGB_props(matr):
    ...
    RGB_props = []
    RGB_props.append(r_comp.mean()/256)
    RGB_props.append(g_comp.mean()/256)
    RGB_props.append(b_comp.mean()/256)
    # RGB_props.append(r_comp.std())
    # RGB_props.append(g_comp.std())
    # RGB_props.append(b_comp.std())
    # RGB_props.append(r_comp.var())
    # RGB_props.append(g_comp.var())
    # RGB_props.append(b_comp.var())
    # RGB_props.append(statistics.mode(r_comp))
    # RGB_props.append(statistics.mode(g_comp))
    # RGB_props.append(statistics.mode(b_comp))
    return RGB_props
...
for exl_row in range(start_row, end_row + 1):
    ...
    gray_prop_V, HSV_prop_v, RGB_prop_v = getLBPPropsForPieceWithOverlap(image_C, blok_size,
                                                                         sheet.cell(exl_row, 1).value[
                                                                         0:len(sheet.cell(exl_row, 1).value) - 4])
...
for i in range(0, len(gray_prop_vect_arr) - 1):
    ...
    print(sheet.cell(i + start_row, 1).value, "-", sheet.cell(i + start_row + 1, 1).value, "   ",
          round(cos_similarity, 2), "    ", dist, "    ", angle / (np.pi / 180), "HSV Cos    ", HSV_cos_similarity,
          "RGB Cos     ", RGB_cos_similarity)
    for_cluster.append([jacard_similarity])
    cos_similarity_arr.append(
        [sheet.cell(i + start_row, 1).value + "-" + sheet.cell(i + start_row + 1, 1).value, cos_similarity])

    # if round(cos_similarity,2) < 0.97:
